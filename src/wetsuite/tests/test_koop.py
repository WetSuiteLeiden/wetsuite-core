...
def test_cvdr_parse_identifier():
    ...
    assert cvdr_parse_identifier("101404_1", prepend_cvdr=True) == (
        "CVDR101404",
        "CVDR101404_1",
    )
    assert cvdr_parse_identifier("CVDR101405_1", prepend_cvdr=True) == (
        "CVDR101405",
        "CVDR101405_1",
    )
    assert cvdr_parse_identifier("CVDR101406", prepend_cvdr=True) == (
        "CVDR101406",
        None,
    )
    assert cvdr_parse_identifier("1.0:101407_1", prepend_cvdr=True) == (
        "CVDR101407",
        "CVDR101407_1",
    )
...
def get_test_data(fn):
    ...
    file = open(
        os.path.join(os.path.dirname(test_koop.__file__), "testfiles", fn), mode="rb"
    )
    filedata = file.read()
    file.close()
    return filedata
...
def test_cvdr_meta():
    ...
    assert (
        meta["identifier"] == "112779_1"
    )  # a value as expected  TODO: check that we normalize this?
...
def test_cvdr_meta_bytes():
    ...
    cvdr_meta(
        b"""<?xml version="1.0" encoding="utf-8"?><cvdr>
...
def test_prefer_types_exclusive():
    ...
    assert prefer_types(
        ...
        first_of=("html", "pdf", "odt"),
    ) == ["metadata", "metadataowms", "xml", "pdf"]
...
def test_prefer_types_one():
    ...
    assert prefer_types(
        ...
        all_of=("metadata", "metadataowms", "xml"),
        first_of=("xml", "html", "pdf", "odt"),
    ) == ["metadata", "metadataowms", "xml"]
...
def test_parse_op_metafile():
    ...
    for fn in (
        ...
        with open(
            os.path.join(os.path.dirname(test_koop.__file__), "testfiles", fn),
            mode="rb",
        ) as f:
            docbytes = f.read()
...
def test_alineas_with_selective_path():
    ...
    """
    tree = wetsuite.helpers.etree.fromstring(
        b"<body><al>test1</al><!-- --><al>test2</al></body>"
    )
