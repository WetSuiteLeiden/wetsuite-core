...
def parse_kst_id(s, debug=False):
    ...
    if len(parts) == 0:
        ...
        #             kst-32168-[3-b2],
        if _is_all_digits( parts[-1] ):          # must be a singular full document number (?) so the first part must be dossiernum
            dossiernum.append( parts.pop(0) )
            ret['docnum'] = parts.pop(0)
            ret['_var'] = '2a'
        elif wetsuite.helpers.strings.has_lowercase_letter( parts[-1] ): # that's the second part of a document numer
            ret['docnum'] = '-'.join(parts)
            ret['_var'] = '2b'
